#!/usr/bin/env python
"""Admin CLI for managing API keys (COMPCHEM_AUTH_MODE=keys).
...
Targets the same key store the server would use: Postgres if COMPCHEM_DATABASE_URL
is set, else the SQLite file at {COMPCHEM_DATA_DIR}/keys.db. `--db PATH` overrides
that and always forces SQLite at the given path, ignoring COMPCHEM_DATABASE_URL.
...
from ypotheto_compchem_mcp.apikeys import KeyStore, SqliteKeyStore, build_key_store
...
def _resolve_key_store(db_override: Path | None) -> KeyStore:
    ...
    return build_key_store(settings)
