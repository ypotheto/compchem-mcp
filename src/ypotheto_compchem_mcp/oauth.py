...
class SigningKeyResolver(Protocol):
    ...
    standing up a real JWKS-serving endpoint. The provider's own JWKS-fetch/
    cache mechanics are PyJWT's well-tested concern, not ours; what's actually
    worth testing here is our own claim validation (issuer/audience/permission/
    sub)."""
...
class OAuthVerifier:
    """Validates OIDC-issued (Kinde, or any RS256-signing OIDC provider) access
    tokens for a single (issuer, audience, required-permission) triple, and
    resolves a valid token to a workspace id.

    The OIDC provider is the authorization server here; this class only ever
    plays the OAuth *resource server* role - there is no /authorize or /token
    endpoint on this side, just signature and claim verification of tokens
    minted elsewhere. Per the MCP authorization spec, audience validation
    (`aud` must match this server) is what stops a token minted for a
    *different* protected API from being replayed here.
    ...
    *present-but-wrong* `aud` is still rejected, so this doesn't fully drop
    ...
    once the configured provider honors `resource`.
    """
    ...
    def verify(self, raw_token: str) -> str | None:
        ...
        if not sub:
            ...
            return None
        return get_workspace_id_from_token(sub)
...
@lru_cache(maxsize=8)
def _cached_oauth_verifier(issuer: str, audience: str, required_permission: str) -> OAuthVerifier:
    return OAuthVerifier(issuer=issuer, audience=audience, required_permission=required_permission)


def build_oauth_verifier(settings: "Settings") -> OAuthVerifier:
    """Cached by (issuer, audience, required_permission) so the request-hot
    path doesn't reconstruct a `jwt.PyJWKClient` (and lose its JWKS cache) on
    every single request, while still picking up a settings change (e.g. a
    test flipping `settings.oauth_issuer`) the moment any of the three values
    actually change. Raises if oauth_issuer/oauth_audience aren't configured -
    callers should only reach this once auth_mode == "oauth" is confirmed."""
    if not (settings.oauth_issuer and settings.oauth_audience):
        raise ValueError(
            "COMPCHEM_AUTH_MODE=oauth requires COMPCHEM_OAUTH_ISSUER and "
            "COMPCHEM_OAUTH_AUDIENCE to both be set"
        )
    return _cached_oauth_verifier(settings.oauth_issuer, settings.oauth_audience, settings.oauth_required_permission)
