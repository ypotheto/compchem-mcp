...
from ypotheto_compchem_mcp.oauth import OAuthVerifier
from ypotheto_compchem_mcp.workspace import get_workspace_id_from_token

_ISSUER = "https://test-tenant.kinde.com"
_AUDIENCE = "https://ypotheto-compchem-mcp.example/mcp"
_PERMISSION = "access:ypotheto-compchem-mcp"
...
def test_valid_token_resolves_to_workspace_id(verifier: OAuthVerifier, keypair: tuple[RSAPrivateKey, Any]):
    private_key, _ = keypair
    token = _make_token(private_key)
    assert verifier.verify(token) == get_workspace_id_from_token("kp_abc123")
...
def test_wrong_issuer_rejected(verifier: OAuthVerifier, keypair: tuple[RSAPrivateKey, Any]):
    private_key, _ = keypair
    token = _make_token(private_key, issuer="https://not-us.kinde.com")
    assert verifier.verify(token) is None


def test_wrong_audience_rejected(verifier: OAuthVerifier, keypair: tuple[RSAPrivateKey, Any]):
    private_key, _ = keypair
    token = _make_token(private_key, audience="https://some-other-api.example/mcp")
    assert verifier.verify(token) is None


def test_missing_aud_claim_accepted_as_stopgap(verifier: OAuthVerifier, keypair: tuple[RSAPrivateKey, Any]):
...
def test_empty_aud_list_accepted_as_stopgap(verifier: OAuthVerifier, keypair: tuple[RSAPrivateKey, Any]):
...
def test_expired_token_rejected(verifier: OAuthVerifier, keypair: tuple[RSAPrivateKey, Any]):
    private_key, _ = keypair
    token = _make_token(private_key, expired=True)
    assert verifier.verify(token) is None


def test_missing_permission_rejected(verifier: OAuthVerifier, keypair: tuple[RSAPrivateKey, Any]):
    private_key, _ = keypair
    token = _make_token(private_key, permissions=["some:other-permission"])
    assert verifier.verify(token) is None


def test_no_permissions_claim_at_all_rejected(verifier: OAuthVerifier, keypair: tuple[RSAPrivateKey, Any]):
    private_key, _ = keypair
    now = time.time()
    token = jwt.encode(
...
def test_missing_sub_rejected(verifier: OAuthVerifier, keypair: tuple[RSAPrivateKey, Any]):
    private_key, _ = keypair
    token = _make_token(private_key, sub=None)
    assert verifier.verify(token) is None


def test_tampered_signature_rejected(verifier: OAuthVerifier):
    other_private_key = rsa.generate_private_key(public_exponent=65537, key_size=2048)
    token = _make_token(other_private_key)  # signed with a key the verifier doesn't trust
    assert verifier.verify(token) is None


def test_garbage_token_rejected(verifier: OAuthVerifier):
